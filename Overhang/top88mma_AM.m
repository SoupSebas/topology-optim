function top88mma_AM(nelx,nely,volfrac,penal,rmin,maxiter,baseplate)
% function top88mma_AM(nelx,nely,volfrac,penal,rmin,maxiter,baseplate)
% adapted from top88 to use MMA OPTIMIZER, and with overhang filter
...
while ((change > convtol) && (iter<maxiter))
  ...
  v = sum(xPrint(:))/(NrEl*volfrac);
  dv = ones(size(xPrint))/(NrEl*volfrac);  
  ...
  x = xnew;  
  xPrintOld=xPrint; % for plotting
  %% update design:
  xPhys(:) = (H*xnew(:))./Hs;  
  xPrint=AMfilter(xPhys,baseplate);

  objhist(iter)=f0val; volhist(iter)=100*mean(xPrintOld(:));

  %% PRINT RESULTS
  fprintf(' It.:%5i Obj.:%11.4f Vol.:%7.3f ch.:%7.3f\n',iter,c, mean(xPrintOld(:)),change); 
  %% PLOT DENSITIES    
  ss=get(0,'screensize');
  ...
  subplot(2,1,1); colormap(gray); imagesc(1-xPrintOld); caxis([0 1]); axis equal; axis tight; title(['Area fraction: ',num2str(volhist(iter)),'%']);  
