function top88mma_compmech(nelx,nely,volfrac,penal,rmin,maxiter)
% function top88mma_compmech(nelx,nely,volfrac,penal,rmin,maxiter)
% adapted from top88 to use MMA OPTIMIZER and optimize a compliant mechanism (maximum output displacement)
% MMA defaults:
asyinitMMA=0.5; 
cMMAvalue=1000; 
convtol = 1e-4;
...
din=1; % input node
dout=2*nelx*(nely+1)+1; % output node
kin=0.1; kout=0.1; % input and output spring stiffnesses
fixeddofs = union(2:2*(nely+1):2*nelx*(nely+1)+2, 2*(nely+1)-[1 2]);  % symmetry at top
alldofs = [1:2*(nely+1)*(nelx+1)];
freedofs = setdiff(alldofs,fixeddofs);
% Extend load and displacement vectors with another vector, for the adjoint solve
F=zeros(2*(nely+1)*(nelx+1),2);
F(din,1)=1;
F(dout,2)=1; % second colum is adjoint load
U=zeros(2*(nely+1)*(nelx+1),2);
% nondesign input and output:
Solid_Nondesign_y=[1:max(1,floor(nely/20))];
Solid_Nondesign_x=[Solid_Nondesign_y nelx+1-Solid_Nondesign_y]-1;
Solid_Nondesign=repmat(Solid_Nondesign_y(:), 1, length(Solid_Nondesign_x)) + nely*repmat(Solid_Nondesign_x,length(Solid_Nondesign_y),1);
Solid_Nondesign=Solid_Nondesign(:);
...
xmin = 0*1e-4;
beta = 1; eta=0.5; % Heaviside projection parameters
...
while ((change > convtol) && (iter<maxiter))
  ...
  %% NONDESIGN
  xPhys(Solid_Nondesign)=1; % solid input and output  

  %% FE-ANALYSIS and adjoint solve
  sK = reshape(KE(:)*(Emin+xPhys(:)'.^penal*(E0-Emin)),64*nelx*nely,1);
  K = sparse(iK,jK,sK); K = (K+K')/2;
  K(din,din) = K(din,din) + kin;  
  K(dout,dout) = K(dout,dout) + kout;

  U(freedofs,:) = K(freedofs,freedofs)\F(freedofs,:);
  Udisp=U(:,1); Adjoint=U(:,2);
  
  %% OBJECTIVE FUNCTION AND SENSITIVITY ANALYSIS
  c=F(:,2)'*Udisp;
  ce = reshape(sum((Adjoint(edofMat)*KE).*Udisp(edofMat),2),nely,nelx);  
  dc = -penal*(E0-Emin)*xPhys.^(penal-1).*ce;  
  v = sum(xPhys(:))/(NrEl*volfrac);
  dv = ones(size(xPhys))/(NrEl*volfrac);    

  %% NONDESIGN
  dc(Solid_Nondesign)=0; 
  dv(Solid_Nondesign)=0; 

  %% FILTERING/MODIFICATION OF SENSITIVITIES
  dxPhys_dxTilde = beta*(1 - (tanh(beta*(xTilde(:)-eta))).^2)/HSdenominator;
  dc(:)=dc(:).*dxPhys_dxTilde;
  dv(:)=dv(:).*dxPhys_dxTilde;  
  dc(:) = H*(dc(:)./Hs);
  dv(:) = H*(dv(:)./Hs);  
  ...
  %% MMA DESIGN UPDATE:                    
  [xmma,ymma,zmma,lam,xsi,MMAeta,mu,zet,s,low,upp] = ...
