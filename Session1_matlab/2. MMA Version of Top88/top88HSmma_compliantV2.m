function top88HSmma_compliantV2(nelx,nely,volfrac,penal,rmin,maxiter)
...
%% BUILD PLATE‐BENDING ELEMENT STIFFNESS  (4-node Kirchhoff plate)
D = E0 * (1/ (1 - nu^2)) * [ 1   nu   0;
                             nu  1    0;
                             0   0  (1-nu)/2 ];
% you can scale by t^3/12 if you like, but we'll absorb that into SIMP

% sample 2×2 Gauss integration to get a 12×12 element matrix Kb0:
[~,Wg,Xg] = gaussquad(2);        % nodes+weights for 2×2
Kb0 = zeros(12);
for i=1:4
  xi = Xg(i,1); eta = Xg(i,2); w  = Wg(i);
  [B,detJ] = plateBmatrix(xi,eta);   % your B-matrix for plate bending
  Kb0 = Kb0 + (B.' * D * B) * detJ * w;
end

% now build edofPlate: each elem has 4 node-indices
nodenrs = reshape(1:(1+nelx)*(1+nely),1+nely,1+nelx);
eidx    = reshape(1:nelx*nely,nelx,nely).';
edofPlate = zeros(nelx*nely,4);
for e=1:nelx*nely
  [i,j] = ind2sub([nelx,nely],e);
  nodes = [nodenrs(j,i), nodenrs(j,i+1), nodenrs(j+1,i+1), nodenrs(j+1,i)];
  edofPlate(e,:) = nodes;
end

% create index vectors for sparse assembly
iKb = repmat(edofPlate(:),1,4);
jKb = repmat(edofPlate(:).',4,1);
iKb = iKb(:);
jKb = jKb(:);
...
%%
% number of bending DOFs = #nodes = (nelx+1)*(nely+1)
Nn = (nelx+1)*(nely+1);
Fz = sparse(Nn,1);
Fz(n_in) = 1;            % apply unit out‐of‐plane load at same corner

% clamp the anchor node in bending (and pin one extra if needed)
fixdofs_b = [n_fix /*, maybe another node to remove rigid tilt*/];
alldofs_b = 1:Nn;
freedofs_b = setdiff(alldofs_b, fixdofs_b);
...
while ((change > convtol) && (iter<maxiter))
  ...
  %% --- assemble & solve out-of-plane bending ---------------------------
% SIMP-scale Kb0 with xPhys^penal exactly as you did the in-plane KE:
De = Emin + xPhys(:).^penal * (E0 - Emin);    % nElem×1
sKb = repmat(De, 12, 1) .* Kb0(:);           % stack 12 entries per element
Kb  = sparse(iKb, jKb, sKb);  Kb = (Kb+Kb')/2;

% solve W:
W = zeros(Nn,1);
W(freedofs_b) = Kb(freedofs_b,freedofs_b) \ Fz(freedofs_b);

% bending compliance & sensitivity per element:
Cb   = Fz.' * W;        % scalar bending compliance
ce_b = reshape(sum((W(edofPlate) * Kb0) .* W(edofPlate), 2), nely, nelx);
dc_b = -penal * (E0-Emin) * xPhys.^(penal-1) .* ce_b;
...
if iter == 1
  ...
  gamma = 0.2;           % tune this so Cz and Cxy are comparable
f0val = -C_xy + gamma * Cb;
df0dx = dc_xy(:)  +  gamma * dc_b(:);
